<div class="space-y-6">
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex items-start justify-between gap-4">
            <div>
                <p class="text-sm font-medium uppercase tracking-[0.2em] text-primary">Réservation</p>
                <h2 class="mt-2 text-2xl font-semibold text-slate-950">Planifier votre séjour</h2>
        ...
        @guest
        {{-- Message affiché aux visiteurs non connectés avant toute réservation. --}}
        ...
        {{-- Message spécifique pour rappeler qu'un administrateur ne peut pas réserver comme un client. --}}
        <div class="mt-6 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
            Le compte administrateur n'a pas accès à l'espace de réservation client.
        </div>
        @else
        {{-- Formulaire de réservation réservé aux utilisateurs clients authentifiés. --}}
        <form wire:submit="save" class="mt-6 grid gap-4 md:grid-cols-2">
            <label class="block">
                <span class="text-sm font-medium text-slate-700">Date d'arrivée</span>
                <input
            ...
            <label class="block">
                <span class="text-sm font-medium text-slate-700">Date de départ</span>
                <input
            ...
            <div class="md:col-span-2 flex flex-col gap-4 rounded-2xl bg-slate-950 px-5 py-4 text-white md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-lg font-semibold">
                        {{ $this->totalNights > 0 ? $this->totalNights.' nuit(s) - '.$this->estimatedTotal.' EUR' : 'Sélectionnez vos dates' }}
                    </p>
                </div>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h3 class="text-lg font-semibold text-slate-950">Réservations récentes</h3>
                <p class="mt-1 text-sm text-slate-600">Aperçu des dernières périodes enregistrées sur ce bien.</p>
            </div>
            <span class="rounded-full bg-slate-100 px-3 py-1 text-sm font-medium text-slate-600">{{ $recentBookings->count() }} enregistrée(s)</span>
        </div>
